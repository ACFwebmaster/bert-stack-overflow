...
flags.DEFINE_integer('max_seq_length', 128,
                     'Maximum sequence length of input sentences.')
...
def get_dataset(filename, tokenizer, max_seq_length, labels_map):
    ...
    '''
    def generator(): return read_csv(filename, tokenizer, max_seq_length, labels_map)
...
def main(_):
    ...
    labels = pd.read_csv(os.path.join(
        FLAGS.data_dir, 'classes.txt'), header=None)
    labels_map = {row[0]: index for index, row in labels.iterrows()}

    # Load dataset, tokenizer, model from pretrained model/vocabulary
    tokenizer = BertTokenizer.from_pretrained('bert-base-cased')
    model = TFBertForMultiClassification.from_pretrained(
        'bert-base-cased', num_labels=len(labels_map), cache_dir=tempfile.mkdtemp())

    # Load dataset, shuffle data, and put into batchs
    train_dataset = get_dataset(os.path.join(
        FLAGS.data_dir, 'train.csv'), tokenizer, FLAGS.max_seq_length, labels_map)
    valid_dataset = get_dataset(os.path.join(
        FLAGS.data_dir, 'valid.csv'), tokenizer, FLAGS.max_seq_length, labels_map)
    test_dataset = get_dataset(os.path.join(
        FLAGS.data_dir, 'test.csv'), tokenizer, FLAGS.max_seq_length, labels_map)
    ...
    optimizer = tf.keras.optimizers.Adam(
        learning_rate=FLAGS.learning_rate*hvd.size(), epsilon=1e-08, clipnorm=1.0)
    ...
    model.compile(optimizer=optimizer, loss=loss, metrics=[
                  metric], experimental_run_tf_function=False)
    ...
    model.fit(train_dataset, epochs=FLAGS.num_epochs, steps_per_epoch=FLAGS.steps_per_epoch //
              hvd.size(), validation_data=valid_dataset, callbacks=callbacks, verbose=verbose)
